...
/**
 * KeyStorage saves the symmetric key wrapped with an Argon2id-derived key from the account password
 * (sym.key), replacing the legacy PKCS#12 store (sym.p12) whose fast KDF allowed cheap brute force.
 * The symmetric key is used to encrypt and decrypt other keys in the key ring and other types of persistence.
 * Legacy files are read transparently and upgraded on the next save.
 */
@Singleton
public class KeyStorage {
    ...
    private byte[] loadKeyBytes(KeyEntry keyEntry, SecretKey secretKey) {
        ...
        try (FileInputStream fis = new FileInputStream(keyFile.getPath())) {
            ...
            fis.read(encodedKey);
            if (!Encryption.isV2Format(encodedKey)) legacyFormatLoaded = true;
