...
public class FormattingUtils {
    ...
    private static final MonetaryFormat nonFiatFormat = new MonetaryFormat().shift(0).minDecimals(8).repeatOptionalDecimals(0, 0);
    ...
    private static String formatCrypto(CryptoMoney crypto, boolean appendCurrencyCode) {
        if (crypto != null) {
            try {
                String res = nonFiatFormat.noCode().format(crypto).toString();
                if (appendCurrencyCode)
        ...
        if (crypto != null) {
            try {
                ...
                if (crypto.getCurrencyCode().equals("BSQ"))
                    res = nonFiatFormat.noCode().minDecimals(2).repeatOptionalDecimals(0, 0).format(crypto).toString();
                else
                    res = nonFiatFormat.noCode().format(crypto).toString();
                if (appendCurrencyCode)
                    return res + " " + crypto.getCurrencyCode();
                else
                    return res;
    ...
    public static MonetaryFormat getMonetaryFormat(String currencyCode) {
        return CurrencyUtil.isFiatCurrency(currencyCode) ? fiatPriceFormat : nonFiatFormat;
    }
}
