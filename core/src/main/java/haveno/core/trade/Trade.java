...
public abstract class Trade extends XmrWalletBase implements Tradable, Model, XmrConnectionListener {
    ...
    public void removeDecryptedDirectMessageListener() {
    ...
    // marks this trade superseded by a new trade for the same id, stopping the protocol error subscriptions and any already-queued removal so they cannot act on the new trade
    ...
    // Completes scheduled protocol error cleanup early: deletes the wallet and cancels the scheduled removal if the
    // deposit txs are verified unpublished. Used when superseded by a new trade for the same id so the wallet is not
    // left behind and the trade is not re-initialized on restart. No-op if not scheduled or the deposits are published.
    ...
    private void removeTradeOnError() {
        synchronized (removeTradeOnErrorLock) {
            ...
            // skip if shutting down, superseded, or this instance is no longer tracked (id lookups can match a newer trade for the same id)
