...
public class StartupShell extends StackPane {
    ...
    // smaller splash logo shown while tall content like the first-run wizard needs the vertical space
    private static final double COMPACT_LOGO_FIT_HEIGHT = 64;
    ...
    /**
     * Switch between a smaller splash logo (compact, for tall content like the first-run wizard)
     * and the full-size one. Leaving compact mode releases the content slot's height lock so the
    ...
    public void setCompactBranding(boolean compact) {
        ...
        if (compact) {
            logo.setFitWidth(0);
            logo.setFitHeight(COMPACT_LOGO_FIT_HEIGHT);
            applySplashLogo();
    ...
    // decode the theme's splash logo at the current fit size for crisp HiDPI rendering; keeps the CSS id as fallback
    private void applySplashLogo() {
        ...
        GUIUtil.setBrandingLogo(logo, preferences, splashBase + "_light_mode.png", splashBase + "_dark_mode.png", logo.getFitWidth(), logo.getFitHeight());
    }
