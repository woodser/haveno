...
public class TacContent {
    ...
    public VBox createRiskPage() {
        ...
        HBox callout = createImportantCallout();
        VBox.setMargin(callout, new Insets(9, 0, 0, 0));
        page.getChildren().addAll(createRiskOverview(), callout, createConfirmationsPanel());
        return page;
    }
    ...
    private HBox createImportantCallout() {
        StackPane iconBox = new StackPane(createIcon(MaterialDesignIcon.ALERT, "1.5em", "tac-agreement-important-icon"));
        iconBox.getStyleClass().add("tac-agreement-important-icon-box");

        Label warning = new Label(Res.get("tacWindow.risk.warning"));
        warning.getStyleClass().add("tac-agreement-important-text");
        warning.setWrapText(true);
        HBox.setHgrow(warning, Priority.ALWAYS);

        HBox callout = new HBox(12, iconBox, warning);
    ...
    private VBox createLegalScrollContent() {
        ...
        HyperlinkWithIcon disputeResolutionLink = new ExternalHyperlink(Res.get("tacWindow.arbitrationSystem"));
        disputeResolutionLink.setOnAction(e -> GUIUtil.openWebPage("https://docs.haveno.exchange/overview/dispute-resolution"));
        content.getChildren().add(disputeResolutionLink);
        return content;
    }
