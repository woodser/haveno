...
public class SeedWordsView extends ActivatableView<GridPane, Void> {
    ...
    // Fail fast on word count, then debounce full seed validation off the UI thread.
    private void validateSeedWords(String seedWords) {
        if (seedValidationTimer != null) seedValidationTimer.stop();
        if (!hasValidWordCount(seedWords)) {
            seedWordsValid.set(false);
            return;
        }
        seedValidationTimer = UserThread.runAfter(() -> new Thread(() -> {
            boolean valid = isSeedValid(seedWords.trim());
            UserThread.execute(() -> {
                if (seedWords.equals(seedWordsTextArea.getText())) seedWordsValid.set(valid);
            });
        }, "ValidateSeedWords").start(), 300, TimeUnit.MILLISECONDS);
    }
